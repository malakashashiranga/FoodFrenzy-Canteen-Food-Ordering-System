$(document).ready(function() {
    $('.buttons').click(function(event) { 
		...
		var formData1 = {};
        formData1['first_name'] = $('#first_name').val();
        formData1['last_name'] = $('#last_name').val();
        formData1['phone_number'] = $('#phone_number').val();
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.success) {
					...
					$('#phoneNumberAlert').text(data.mobileNumberAlert);
					...
					if (data.alertContent) {
						if (data.buttons === 'logout_acc_btn') {
							defPageCreateFunc();
