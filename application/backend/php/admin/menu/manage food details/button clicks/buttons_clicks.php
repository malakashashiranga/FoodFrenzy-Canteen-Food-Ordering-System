...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	if (isset($_POST['buttonId']) || isset($_POST['buttonClass'])) {
		$buttonId = $_POST['buttonId'];
        $buttonClass = $_POST['buttonClass'];
		
		$foodDetails = $_SESSION['food_details'];
		$oldFoodName = $foodDetails['foodName'];
		$oldPhotoPath = $foodDetails['photoPath'];
		$oldAvailability = $foodDetails['availability'];
		$foodNumber = $foodDetails['foodNumber'];
		
		if (isset($_SESSION['foodManageButtonProcess']) && isset($_SESSION['foodManageButtonId']) && isset($_SESSION['foodManageBtnProcessStep1'])) {
			
			$prevFoodManageButtonProcess = $_SESSION['foodManageButtonProcess'];
			...
			if ($conn_status === "successfully") {
				...
				if ($buttonId === 'cancel_button' && $buttonClass === 'ajax-button' && $prevFoodManageButtonProcess === 'edit_food' && $foodManageButtonId === 'edit_button') {
					$response['buttons'] = 'food_change_cancel_btn';
					
				}elseif ($buttonId === 'submit_button' && $buttonClass === 'ajax-button' && $prevFoodManageButtonProcess === 'edit_food' && $foodManageButtonId === 'edit_button') {
					
					$foodName = strtolower($_POST['food_name']);
					$discountPrice = $_POST['discount_price'];
					$nonDiscountPrice = $_POST['non_discount_price'];
					$details = strtolower($_POST['food_details']);
					$foodCategory = $_POST['food_category'];
					$availability = $_POST['availability'];
					
					$allowedTypes = array('image/jpeg', 'image/jpg', 'image/png', 'image/ico');

					if (!isset($_FILES['imageUpload']['type']) && !in_array($_FILES['imageUpload']['type'], $allowedTypes)) {
					...
					$foodExistCheck = "SELECT food_name FROM foods WHERE food_name = ? AND food_name!= ? AND availability != 'deleted'";
			
					if ($stmt = $conn->prepare($foodExistCheck)) {
						$stmt->bind_param("ss", $foodName, $oldFoodName);
						$stmt->execute();
						...
						if ($result->num_rows === 0) {
							...
							if ($oldFoodName !== $foodName) {
								if (is_dir($old_upload_dir)) {
									if (rename($old_upload_dir, $new_upload_dir)) {
										$newFileName = $foodName . ".jpg";
										$newFilePath = $new_upload_dir . $newFileName;

										if (file_exists($new_upload_dir . $oldFoodName . '.jpg')) {
											if (rename($new_upload_dir . $oldFoodName . '.jpg', $newFilePath)) {
												$response['consoleAlert'] = 'Successfully renamed food directory and image file.';
											} else {
												$response['success'] = false;
												$response['consoleAlert'] = 'Error renaming image: Unable to rename the file.';
											}
										} else {
											$response['consoleAlert'] = 'Successfully renamed food directory, but image file not found.';
										}
									} else {
										$response['success'] = false;
										$response['consoleAlert'] = 'Error renaming directory: Unable to rename the directory.';
									}
								} else {
									$response['success'] = false;
									$response['consoleAlert'] = 'Error renaming directory: Source directory does not exist.';
								}
							}
							...
							if (!empty($_FILES['imageUpload']['tmp_name'])) {
								...
								$new_file_path = $new_upload_dir . $new_file_name;
							...
							$updateFoodDetails = "UPDATE foods SET 
												food_name = ?,
												discount_price = ?,
												non_discount_price = ?,
												category = ?,
							...
							if ($stmt = $conn->prepare($updateFoodDetails)) {
								$stmt->bind_param("ssssssss", $foodName, $discountPrice, $nonDiscountPrice, $foodCategory, $availability, $new_file_name, $details, $oldFoodName);
								$stmt->execute();
								$stmt->close();
								
								
								if ($oldAvailability === 'available' && $availability === 'unavailable') {
								...
								$_SESSION['food_details'] = array(
									'foodName' => capitalizeAfterSpaceOrSpecialChar($foodName),
									'discountPrice' => $discountPrice,
									'nonDiscountPrice' => $nonDiscountPrice,
									'category' => $foodCategory,
									'availability' => $availability,
									'photoPath' => $new_file_name,
									'details' => ucfirst(capitalizeAfterDot($details))
								);

								$response['buttons'] = 'food_change_save_btn';
								$response['alert'] = 'Food details successfully updated.!';
								
							} else {
								$response = array(
									'success' => false,
									'alert' => 'Error in the updateFoodDetails query.',
									'error_page' => true
								);
							}	
						} else {
							$response['success'] = false;
							$response['foodNameAlert'] = '*Food name is already exist.';
						}
					}
				} else {
					$funcArray = firstStepButtonClick($buttonId, $buttonClass);
			
					if(!isset($funcArray['else_alert'])) {
						if ($buttonId === 'delete_button') {
...
function firstStepButtonClick($buttonId, $buttonClass) {
    $funcArray = array(); 

    if ($buttonId === 'edit_button' && $buttonClass === 'ajax-button') {
        $foodManageButtonProcess = 'edit_food';
        $foodManageButtonId = 'edit_button';
        $funcArray['buttons'] = 'edit_food_btn';
	} elseif ($buttonId === 'delete_button' && $buttonClass === 'ajax-button') {
        $foodManageButtonProcess = 'delete_food';
        $foodManageButtonId = 'delete_button';
        $funcArray['alertContent'] = true;
		$funcArray['alert'] = 'Do you want to permenantly delete this food item?';
        $funcArray['buttons'] = 'delete_food_btn';
    } else {
		$funcArray['else_alert'] = true;	
    }

    $_SESSION['foodManageButtonProcess'] = $foodManageButtonProcess;
    $_SESSION['foodManageButtonId'] = $foodManageButtonId;
    $_SESSION['foodManageBtnProcessStep1'] = 'complete';
