...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['searchTerm'])) {
        ...
        if ($conn_status === "successfully") {
			...
			$state = "pending";
			if ($searchTerm === '') {
				$selectPendingOrders = "SELECT o.order_id, u.user_id, u.email, o.ordered_date, o.ordered_time
										FROM orders o
			...
			if ($stmt = $conn->prepare($selectPendingOrders)) {
				...
				if ($result->num_rows > 0) {
					...
					while ($row = $result->fetch_assoc()) {
						$ordersDetail = array(
							...
							'email' => $row['email'],
							'placedDate' => $row['ordered_date'],
							'placedTime' => $row['ordered_time']
						);
					...
					if ($searchTerm === '') {
						...
						$response['content'] = "No pending orders yet!";
					} else {
						$response['noFoods'] = true;
						$response['content'] = "No pending order found with the id '$searchTerm'";
					}
				}
				$stmt->close();
			} else {
                $response = array(
                    'success' => false,
                    'alert' => 'Error in selectPendingOrders query.',
                    'error_page' => true
                );
            }
        } else {
