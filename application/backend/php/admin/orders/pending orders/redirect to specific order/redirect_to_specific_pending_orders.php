...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    if (isset($_GET['orderID'])) {
        ...
        if ($conn_status === "successfully") {			
			...
			$retriveOrderDetails = "SELECT u.photo_path, u.user_id, u.first_name, u.last_name, u.email, u.mobile_number, o.ordered_date, o.ordered_time, o.pay_method, SUM(f.discount_price * cf.quantity) AS total_price
									FROM orders o
			...
			if ($stmt = $conn->prepare($retriveOrderDetails)) {
				...
				if ($result->num_rows > 0) {
					...
					if($pay_method === 'wallet') { 
						$payMethod = 'Already paid using wallet';
					} elseif($pay_method === 'cash') {
						$payMethod = 'To be paid with cash';
					}
					...
					// Fetch and store the food details separately
					...
					header('Location: /FoodFrenzy-Canteen-Food-Ordering-System/application/frontend/php/pages/admin/orders/pending orders/specific pending order/specific_pending_order.php');
				} else {
				...
				$errorMessage = "Error in retriveOrderDetails query.";
				redirectToErrorPage($errorMessage);    	
			}
		} else {
...
function redirectToErrorPage($errorMessage) {
    ...
    $goBackURL = "/FoodFrenzy-Canteen-Food-Ordering-System/application/frontend/php/pages/admin/orders/pending orders/order list/pending_orders.php"; 
