        ...
        if ($conn_status === "successfully") {			
			...
			if ($stmt = $conn->prepare($retriveOrderDetails)) {
				...
				if ($result->num_rows > 0) {
					...
					header('Location: /FoodFrenzy/application/frontend/php/pages/admin/orders/pending orders/specific pending order/specific_pending_order.php');
				} else {
...
function redirectToErrorPage($errorMessage) {
    ...
    $goBackURL = "/FoodFrenzy/application/frontend/php/pages/admin/orders/pending orders/order list/pending_orders.php"; 
