	...
	if (isset($_POST['buttonId']) && isset($_POST['buttonClass']) && isset($_POST['action'])) {
		...
		if (isset($_SESSION['settingButtonProcess']) && isset($_SESSION['settingButtonId']) && isset($_SESSION['settingBtnProcessStep1'])) {
			...
			if ($conn_status === "successfully") {
				...
				if ($buttonId === 'conf_change_prof_info' && $buttonClass === 'buttons' && $action === 'conf_prof_info' && $prevSettingButtonProcess === 'prof_info' && $prevSettingButtonId === 'prof_info') {

					$formData1 = $_POST['formData1'];

					$newFirstName = strtolower($formData1['first_name']);
					$newLastName = strtolower($formData1['last_name']);
					$newPhone = strtolower($formData1['phone_number']);
					...
					if ($response['success'] === true) {
						...
						if ($stmt = $conn->prepare($updateUserData)) {
							$stmt->bind_param("ssiss", $newFirstName, $newLastName, $newPhone, $newAddress, $UserId);
							$stmt->execute();
							$stmt->close(); 
				...
				} elseif ($buttonId === 'conf_change_pword' && $buttonClass === 'buttons' && $action === 'conf_password_ch' && $prevSettingButtonProcess === 'password_ch' && $prevSettingButtonId === 'password_ch') {
...
function firstStepButtonClick($buttonId, $buttonClass, $action) {
    ...
    if ($buttonId === 'prof_info' && $buttonClass === 'buttons' && $action === 'prof_info') {
        $settingButtonProcess = 'prof_info';
        $settingButtonId = 'prof_info';
        $funcArray['buttons'] = 'prof_info_btn';
	} elseif ($buttonId === 'email_ch' && $buttonClass === 'buttons' && $action === 'email_ch') {	
    ...
    } elseif ($buttonId === 'password_ch' && $buttonClass === 'buttons' && $action === 'password_ch') {
        $settingButtonProcess = 'password_ch';
        $settingButtonId = 'password_ch';
        $funcArray['buttons'] = 'password_ch_btn';
    } elseif ($buttonId === 'logout_acc' && $buttonClass === 'buttons' && $action === 'logout_acc') {
