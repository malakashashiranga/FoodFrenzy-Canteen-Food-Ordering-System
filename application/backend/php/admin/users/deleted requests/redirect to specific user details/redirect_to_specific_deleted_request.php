...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    if (isset($_GET['userID'])) {
        ...
        if ($conn_status === "successfully") {			
			...
			$retriveUserDetails = "SELECT u.photo_path, u.user_id, u.first_name, u.last_name, u.email, u.mobile_number, u.registered_date, u.registered_time, ul.last_sign_date, ul.last_sign_time, acs.last_response_date, acs.last_response_time, acs.activeness, uw.current_balance
									FROM users u
			...
			if ($stmt = $conn->prepare($retriveUserDetails)) {
				...
				if ($result->num_rows > 0) {
					...
					$_SESSION['specific_user_details'] = array(
						...
						'walletBalance' => "Rs. " .$row['current_balance'],
