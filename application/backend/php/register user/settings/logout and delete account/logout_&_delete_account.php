    ...
    if (
        ...
        if ($conn_status === "successfully") {
            ...
            if (
                ...
                $prevSettingButtonProcess === 'delete_acc' &&
                $prevSettingButtonId === 'delete_button' &&
                $settingBtnProcessStep1 === 'complete'
            ) {
                $updateActivenessQuery = "UPDATE active_sessions SET activeness = 'deactivated' WHERE user_id = ?";
    
                if ($stmt = $conn->prepare($updateActivenessQuery)) {
                    $stmt->bind_param("s", $UserId);
                    $stmt->execute();
                    $stmt->close(); 
					
					$updateTokenValidity = "UPDATE users_last_login SET token_validity = 'unvalid' WHERE user_id = ?";
					if ($stmt = $conn->prepare($updateTokenValidity)) {
						...
						$response['alertType'] = 'delete_acc';
						// $response['alert'] = "Your account was deleted successfully.<br/>Check your e-mail for further details.";
					} else {
						$response['success'] = false;
						$response['alert'] = 'Error in account deletion user logout statement';
